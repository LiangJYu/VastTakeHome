...
MiningTruck::MiningTruck() : state(TruckState::mining),
...
unsigned int MiningTruck::compute_mining_time() const {
    srand(69);
    return TimeConstants::min_mining_time + std::rand() % TimeConstants::mining_time_range;
}
...
//        std::cout << std::format("advance_state_and_event {} ", t_now);
        ...
        if (state == TruckState::transit_to_mine) {
            ...
            t_completion = t_state_start + TimeConstants::transit_time;
        }
        else if (state == TruckState::transit_to_unload) {
            auto unload_time = TimeConstants::unload_time;
